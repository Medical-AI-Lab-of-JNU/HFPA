...
from visualization.MolFormer.utils import normalize_smiles
import torch
import shutil
...
class TestBert(nn.Module):
    def __init__(
        self, vocab, model_path=None, rotate=False, device="cpu"
    ):
        if model_path == None:
            assert False
        super().__init__()
...
def get_bert(config, tokenizer):
    bert_model = TestBert(
        tokenizer.vocab, config.pretrain_model_root, rotate=config.rotate, device=config.device
    ).to(config.device)
...
def get_full_attention(bert_model,tokenizer,molecule,config):
    ...
    if loader is not None:
        ...
        with torch.no_grad():
            ...
            if config.mask is True:
                ...
                att_mask = FullMask(
                    torch.ones(torch.tensor(batch_ids["input_ids"]).size(), dtype=bool).to(device),
                    device=device,
                )
            ...
            return attention_mask, embeddings,raw_tokens
