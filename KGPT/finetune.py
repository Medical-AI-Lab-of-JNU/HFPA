...
from KGPT.utils import set_random_seed
import argparse
import torch
...
from KGPT.model.light import LiGhTPredictor as LiGhT
from KGPT.trainer.scheduler import PolynomialDecayLR
from KGPT.trainer.finetune_trainer import Trainer
from KGPT.trainer.evaluator import Evaluator
from KGPT.trainer.result_tracker import Result_Tracker
from KGPT.model_config import config_dict
...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default='../Pretrain_checkpoints/KGPT.pth')
    parser.add_argument("--dataset", type=str, default='toxcast')
    parser.add_argument("--data_path", type=str, default='../Data/Kgpt_FineTuneData/')
    parser.add_argument("--dataset_type", type=str, default='classify')
...
def finetune(args):
    ...
    train_dataset = MoleculeDataset(root_path=args.data_path, dataset=args.dataset, dataset_type=args.dataset_type,
                                    split_name=f'{args.split}', split='train')
    val_dataset = MoleculeDataset(root_path=args.data_path, dataset=args.dataset, dataset_type=args.dataset_type,
                                  split_name=f'{args.split}', split='val')
    test_dataset = MoleculeDataset(root_path=args.data_path, dataset=args.dataset, dataset_type=args.dataset_type,
                                   split_name=f'{args.split}', split='test')
    train_loader = DataLoader(train_dataset, batch_size=128, shuffle=True, num_workers=args.n_threads,
                              worker_init_fn=seed_worker, generator=g, drop_last=True, collate_fn=collator)
    val_loader = DataLoader(val_dataset, batch_size=128, shuffle=False, num_workers=args.n_threads,
                            worker_init_fn=seed_worker, generator=g, drop_last=False, collate_fn=collator)
    test_loader = DataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=args.n_threads,
                             worker_init_fn=seed_worker, generator=g, drop_last=False, collate_fn=collator)
    ...
    model.predictor = get_predictor(d_input_feats=config['d_g_feats'] * 3, n_tasks=train_dataset.n_tasks, n_layers=2,
    ...
    lr_scheduler = PolynomialDecayLR(optimizer, warmup_updates=args.n_epochs * len(train_dataset) // 32 // 10,
                                     tot_updates=args.n_epochs * len(train_dataset) // 32, lr=args.lr, end_lr=1e-9,
                                     power=1)

    if args.dataset_type == 'classify':
    ...
    summary_writer = None
    trainer = Trainer(args, optimizer, lr_scheduler, loss_fn, evaluator, result_tracker, summary_writer, device=device,
                      model_name='LiGhT',
